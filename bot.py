...
from db_utils import get_session, get_fights_for_event, update_fight_odds_batch, open_event_for_bets, get_draft_events, create_event_from_api, get_event_by_id, get_events_for_odds_edit, get_all_users
...
class AdminStates(StatesGroup):
    ...
    # всё, больше ничего не нужно, waiting_for_confirmation не используется
...
@dp.callback_query(lambda c: c.data.startswith("admin_") 
                   ...
                   and c.data != "admin_announce")  # <-- ДОБАВЬ ЭТО!
...
@dp.callback_query(lambda c: c.data == "admin_announce")
async def process_admin_announce(callback: CallbackQuery, state: FSMContext):
    print("🔔 Обработчик admin_announce ВЫЗВАН!")
...
@dp.message(AdminStates.waiting_for_announcement, F.content_type.in_({
...
@dp.callback_query(lambda c: c.data == "announcement_confirm")
...
@dp.callback_query(lambda c: c.data == "announcement_cancel_final")
